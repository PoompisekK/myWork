...
export class HttpService {
  ...
  public httpGet<T>(url: string,
    searchParams?: string | URLSearchParams | { [key: string]: any | any[]; },
  ...
  public httpGetObservable<T>(url: string, searchParams?: string | URLSearchParams | {
    ...
    let headers = new Headers({
      'Content-Type': RequestContentType.APPLICATION_JSON, // NorrapatN: Experiment for iOS Security
      'App-Request-Version': this.getRequestAppVersion(),
      'App-Request-ID': this.getRequestAppId(),
  ...
  public httpPostObservable<T>(url: string, searchParams?: { [key: string]: any }, bodyParams?: { [key: string]: any },
    ...
    switch (contentType) {
      ...
      default:
        ...
        body = bodyParams; // For PostJson NativeHTTP
    }
   ...
   */
  public httpJSONP<T>(url: string,
    params?: { [key: string]: any },
    contentType?: RequestContentType): Observable<T> {
  ...
  */
  ...
  private getRequestAppId(): string {
    return "com.avalant.workforce";
  }

  private getRequestAppVersion(): string {
    let appNo = (AppConstant.APP_VERSION || '').split(' ');
    return (appNo && appNo.length > 0) ? appNo[0] : '';
  }
}
