/**
 * Mask Function for 'text-mask' module
 * 
 * @description Before use this you just run 'npm i text-mask-addons --save'
 * 
 * @see https://github.com/text-mask/text-mask/tree/master/addons#createnumbermask
 * @see https://github.com/text-mask/text-mask/tree/master/addons/#readme
 * @since Wed Jun 28 2017
 */

// import createNumberMask from 'text-mask-addons/dist/createNumberMask'

export const currencyBahtMask = createNumberMask({
  // prefix: '฿',
  prefix: '',
  suffix: '',
  // allowDecimal: true,
...
export default function createNumberMask({
  ...
  function numberMask(rawValue = emptyString) {
    ...
    if (
      ...
      return prefix.split(emptyString).concat([<any>digitRegExp]).concat(suffix.split(emptyString));
    } else if (
      rawValue === decimalSymbol &&
      allowDecimal
    ) {
      return prefix.split(emptyString).concat(['0', decimalSymbol, <any>digitRegExp]).concat(suffix.split(emptyString));
    }
