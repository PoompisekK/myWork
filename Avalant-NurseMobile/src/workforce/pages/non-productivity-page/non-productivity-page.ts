...
import { NonCreatePage } from './non-create-pages/non-create-page';

@Component({
	selector: 'non-productivity-page',
	templateUrl: 'non-productivity-page.html',
	animations: [
		AnimateCss.peek()
	]
})
export class NonProductivityPage implements OnInit {
	...
	private selectType(_shift) {
		...
		if (_shift == 'meeting') {
			this.shiftType = 'meeting';
			this.Meeting = 'select';
			this.Training = '';
		} else if (_shift == 'training') {
			this.shiftType = 'training';
			this.Meeting = '';
			this.Training = 'select';
		} 
	...
	private gotoCreateLeave() {
		var leaveType = "";
		if (this.Meeting == 'select') {
			this.navCtrl.push(NonCreatePage);
		} else if (this.Training == 'select') {
            this.navCtrl.push(NonCreatePage);
        }
	}
